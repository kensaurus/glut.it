...
def rnn_cell_forward(xt, a_prev, parameters):
    ...
    a_next = np.tanh(np.matmul(Waa,a_prev)+np.matmul(Wax,xt)+ba) #ba is broadcasted
    # compute output of the current cell using the formula given above
    yt_pred = softmax(np.matmul(Wya,a_next)+by) #by is broadcasted   
...
def lstm_cell_forward(xt, a_prev, c_prev, parameters):
    ...
    # print(ft.shape, c_prev.shape, it.shape, cct.shape)
    c_next = ft*c_prev+it*cct
    ot = sigmoid(np.dot(Wo,concat)+bo)
    # print(c_next.shape,ot.shape)
...
def rnn_cell_backward(da_next, cache):
    ...
    # dtanh = 1-tanh(Wax*xt + Waa*a_prev + b)^2
...
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    concat = np.concatenate((a_prev, xt), axis=0)
    dWf = np.dot(dft, np.transpose(concat))
    dWi = np.dot(dit, np.transpose(concat))
    dWc = np.dot(dcct, np.transpose(concat))
    dWo = np.dot(dot, np.transpose(concat))
    ...
    wft = np.transpose(ft[:,n_a :])
    wit = np.transpose(it[:,n_a :])
    wcct = np.transpose(cct[:,n_a :])
    wot = np.transpose(ot[:,n_a :])
    da_prev = np.dot(wft,dft)+np.dot(wit,dit)+np.dot(wot,dot)
    dc_prev = dc_next*ft+ot*(1-np.tanh(c_next**2))*ft*da_next
    dxt = np.dot(wft,dft)+np.dot(wit,dit)+np.dot(wcct,dcct)+np.dot(wot,dot)
...
def lstm_backward(da, cache):
    ...
    for t in reversed(range(T_x)):
        ...
        gradients = lstm_cell_backward(da[:,:,t]+da_prevt, caches[t])
